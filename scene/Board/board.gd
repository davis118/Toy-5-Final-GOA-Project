	...
	for r in range(board_size):
		...
		for c in range(board_size):
			if (value == board_size*board_size):
				...
				if tile_scene:
					var tile_instance = tile_scene.instance() # Corrected line
...
func slide_row(row, dir, limiter):
	...
	if dir == 1:
		...
		pre.pop_back()
		var post = row.slice(empty_index, row.board_size())
		post.pop_front()
		...
		post.pop_front()
		var end = row.slice(limiter, row.board_size() - 1)
		end.pop_front()
...
func slide_column(col, dir, limiter):
	...
	if dir == 1:
		...
		pre.pop_back()
		var post = col.slice(empty_index, col.board_size() - 1)
		post.pop_front()
		...
		post.pop_front()
		var end = col.slice(limiter, col.board_size() - 1)
		end.pop_front()
		return pre + post + [0] + end
